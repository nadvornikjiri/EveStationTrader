...
def test_fetch_regional_history_requests_each_type_and_normalizes_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    FakeHttpxClient.responses = {
        ("/markets/10000002/history/", (("type_id", 34),)): FakeResponse(
            [
                {
                    "date": "2026-03-20",
                    "average": 100,
        ...
        ("/markets/10000002/history/", (("type_id", 35),)): FakeResponse(
            [
                {
                    "date": "2026-03-19",
                    "average": 200.5,
...
def test_fetch_regional_history_skips_missing_history_types(monkeypatch: pytest.MonkeyPatch) -> None:
    FakeHttpxClient.instances.clear()
    FakeHttpxClient.responses = {
        ("/markets/10000002/history/", (("type_id", 34),)): FakeResponse(
            [
        ...
        ("/markets/10000002/history/", (("type_id", 999999),)): FakeResponse(
            {"error": "No data found"},
            status_code=404,
        ),
    }
    ...
    history = EsiClient().fetch_regional_history(10000002, [34, 999999])

    assert [row["type_id"] for row in history] == [34]
