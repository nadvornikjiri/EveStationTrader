...
class FoundationDataService:
    ...
    def bootstrap(self, session: Session) -> FoundationSeedResult:
        ...
        self._seed_regions(session, result)
        region_lookup = self._build_region_lookup(session)

        self._seed_systems(session, result, region_lookup)
        system_lookup = self._build_system_lookup(session)

        self._seed_locations(session, result, region_lookup, system_lookup)
        self._seed_items(session, result)
        self._seed_tracked_structures(session, result, region_lookup, system_lookup)
        self._seed_defaults(session, result)

        session.commit()
        return result

    def _seed_regions(self, session: Session, result: FoundationSeedResult) -> None:
        for region in self.seed_source.regions():
    ...
    def _seed_systems(
        ...
        for system in self.seed_source.systems():
    ...
    def _seed_locations(
        ...
        for station in self.seed_source.stations():
            self._seed_station(session, result, station, region_lookup, system_lookup)

        for structure_id, metadata in self.seed_source.structure_locations().items():
    ...
    def _seed_items(self, session: Session, result: FoundationSeedResult) -> None:
        for item in self.seed_source.items():
    ...
    def _seed_tracked_structures(
        ...
        for structure_id, tracked_seed in tracked_structures.items():
